...
**NOTE:** Other add-ons installed in Blender may throw warnings or errors when running this command. As long as the console shows `building: cyanic_toolbox-<version number>-<platform>.zip` followed by `complete` you should have a completed build of the add-on ready to be installed. If the warnings or errors do prevent the build from completing then disable those other addons, then run step 3 again.